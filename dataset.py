...
def get_data_dict(video_dir, label_dir, feature_dir_list,
    ...
    for video in tqdm(video_list):
        ...
        # shapes_1 = [feature_list[i].shape[1] for i in range(len(feature_list))]
        # min_shape = min(shapes_1)
        # if min_shape != max(shapes_1):
        #     logging.info("Feature {} cropped from {} to {}".format(video, max(shapes_1), min_shape))
        # feature_list = [feature_list[i][:, :min_shape, :] for i in range(len(feature_list))]
        # if sum(bools) > 0:
        #     print("features of unequal length, ignoring")
        #     continue
        ...
        print('Feature List Processed: {} - {}'.format(video, [i[0].shape for i in feature_list]))
