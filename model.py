...
class ChannelMLPModule(nn.Module):
    ...
    def __init__(self, input_dim, middle_dim, output_dim, num_layers, activation="relu"):
...
class UnifiedSkillNet(nn.Module):
    ...
    def __init__(self, input_dim_list, embedding_dim_list, instance_norm_flags,
                 ...
                 use_temp_attn=False, activation="relu"):
    ...
    def forward(self, x, weighting=False):
        ...
        # scores = torch.cat([torch.mean(i.unsqueeze(2)).unsqueeze(0).unsqueeze(0).unsqueeze(0) for i in scores], dim=2)  # (N, S, P)
    ...
    def get_score_attention(self, x):
        ...
        # N = 1, F = feature dim, T = sequence length
        ...
        if self.use_temp_attn:
            temporal_attention = self.temp_attend_module(x_cat) if self.temp_attend_module else torch.zeros_like(scores[0])
